...
const geometry = fitStaffGeometry([
  ...
  { stepIndex: 0, page: 0, system: 0, staffNumber: 1, diatonicIndex: Pitch.parse('E4').diatonicIndex, y: 145.5 },
...
describe('buildOverlayShapes', () => {
  ...
  it('scales with the drawing, so zoom moves nothing out of place', () => {
    const zoomed = fitStaffGeometry([
      { stepIndex: 0, page: 0, system: 0, staffNumber: 1, diatonicIndex: Pitch.parse('C4').diatonicIndex, y: 311 },
      { stepIndex: 0, page: 0, system: 0, staffNumber: 1, diatonicIndex: Pitch.parse('E4').diatonicIndex, y: 291 },
    ]);
    if (zoomed === null) {
      throw new Error('expected geometry');
    }
