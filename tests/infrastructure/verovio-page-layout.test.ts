...
describe('a page Verovio drew, read off the drawing', () => {
  ...
  it('reads the number printed over a bar: what it says, on its bar line, above the staff, and how large', () => {
    // Every second bar is numbered, so the first bar is not and the second is.
    const space = ((first?.staves[0]?.bottom ?? 0) - (first?.staves[0]?.top ?? 0)) / 4;

    expect(first?.number).toBeNull();
    expect(second?.number?.text).toBe('2');
    expect(Math.abs((second?.number?.x ?? Infinity) - (second?.left ?? 0))).toBeLessThan(space);
    expect(second?.number?.y).toBeLessThan(system?.top ?? -Infinity);
    // A figure about as tall as two spaces of the staff, as engravers set one.
    expect(second?.number?.size).toBeGreaterThan(space);
    expect(second?.number?.size).toBeLessThan(4 * space);
  });
});
  ...
  it('is added to everything on the page, lines and heads alike', () => {
    ...
    const svg = new DOMParser().parseFromString(
      '<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">' +
        ...
        '</g></g></g></g>' +
        // A system's first number, which Verovio writes with no x: it stands
        // where the system begins.
        '<g class="mNum autogenerated"><text y="80" text-anchor="middle" font-size="0px">' +
        '<tspan class="text"><tspan font-size="36px">17</tspan></tspan></text></g>' +
        '</g></g></g></svg></svg>',
      'image/svg+xml',
    );
